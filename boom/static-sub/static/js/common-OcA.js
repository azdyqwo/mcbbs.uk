...
function showMenu(v) {
	...
	if(cover){ 
		$(menuid + '_cover').style.opacity = 0.5;
		$(menuid + '_cover').style.pointerEvents = "all";
	}
	if(fade) {
		var O = 0;
	...
	if(menuObj.style.display=='none')
		...
		menuObj.style.transition = 'opacity 200ms ease';
		menuObj.style.opacity = 100;
		menuObj.style.pointerEvents = 'all';
...
function setMenuPosition(showid, menuid, pos) {
	...
	if(showObj) {
		...
		//二次开发，适配新版框架
		/*
		if(menuObj.parentNode.localName != 'body') {
		...
		var hide = function() {
			if(menuObj.cache) {
				if(menuObj.style.visibility != 'hidden') {
					...
					if(menuObj.cover){ 
						$(attr + '_cover').style.opacity = 0;
						$(attr + '_cover').style.pointerEvents = "none";
						setTimeout(function(){ if($(attr + '_cover').style.opacity == 0) $(attr + '_cover').style.display = 'none'; },500); 
					}
					setTimeout(function(){ if(menuObj.style.opacity == 0) menuObj.style.display = 'none'; },800); 
				}
			} else {
